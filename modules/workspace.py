...
def get_service():
    """Łączy się z Google Drive API korzystając z Twoich Secrets"""
    try:
...
def get_user_folder(service, email):
    """Szuka lub tworzy folder użytkownika WEWNĄTRZ Twojego folderu głównego"""
    folder_name = f"Konto_{email}"
    # Szukamy folderu o nazwie e-maila, który jest dzieckiem PARENT_FOLDER_ID
    ...
    if files:
        ...
        # Jeśli nie ma, tworzymy nowy i przypisujemy mu rodzica (Ciebie)
        folder_metadata = {
            'name': folder_name,
...
def sync_files(email, uploaded_files):
    """Wysyła pliki z aplikacji na Dysk Google"""
    service = get_service()
    ...
    for uf in uploaded_files:
        ...
        # Sprawdzamy, czy plik już istnieje, żeby go nadpisać (update) zamiast duplikować
...
def load_workspace(email):
    """Pobiera pliki z Dysku Google do pamięci aplikacji"""
    service = get_service()
    if not service: return []
    ...
    for f_info in files_info:
        try:
            ...
            fh.seek(0)
            # Tworzymy obiekt CloudFile, który load_data rozpozna jako zwykły plik
            cloud_files.append(CloudFile(fh.read(), f_info['name']))
        except Exception as e:
            print(f"Pominięto plik {f_info['name']} z powodu błędu: {e}")
